...
if (!"null".equals(terminationDate)) {
    Date tempTerminationDate = dateFormat.parse(terminationDate);
    // Check if termination date is before joining date
    if (tempTerminationDate.before(this.joiningDate)) {
        throw new InvalidInputException("Termination date cannot be before joining date.");
    }
    this.terminationDate = tempTerminationDate;
} else {
